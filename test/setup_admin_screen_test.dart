...
void main() {
  testWidgets('renders SetupAdminScreen with all elements and localized strings', (
    tester,
  ) async {
    ...
    expect(find.text('Welcome to Cue. Create your administrator account to initialize the workspace.'), findsOneWidget);
    ...
    expect(
      find.text('Credentials are encrypted and saved securely in the database. No plaintext password is stored in .env.'),
      findsNothing,
    );
  });
  ...
  testWidgets('renders Chinese localization correctly', (tester) async {
    ...
    expect(
      find.text('凭据已安全哈希并存储在数据库中，不会在本地 .env 文件中保存明文密码。'),
      findsNothing,
    );
  });
  ...
  testWidgets('validates required fields, password length and password match', (
    ...
    for (final invalid in ['plainaddress', 'admin@', 'admin@cue', 'admin@.local', 'admin@cue..local']) {
      await tester.enterText(find.byKey(const Key('setup-email')), invalid);
      await tester.tap(find.byKey(const Key('setup-submit')));
      await tester.pumpAndSettle();
      expect(find.text('Enter a valid email'), findsOneWidget, reason: 'Failed for $invalid');
      expect(submittedEmail, isNull);
    }

    // Enter valid email (with whitespace to verify trimming) and short password
    await tester.enterText(find.byKey(const Key('setup-email')), '  admin@cue.local  ');
    ...
    await tester.enterText(find.byKey(const Key('setup-password')), 'password123');
    await tester.enterText(find.byKey(const Key('setup-confirm-password')), 'different123');
    await tester.tap(find.byKey(const Key('setup-submit')));
    await tester.pumpAndSettle();
    expect(find.text('Passwords do not match'), findsOneWidget);

    // Enter matching confirm password and submit
    await tester.enterText(find.byKey(const Key('setup-confirm-password')), 'password123');
    await tester.tap(find.byKey(const Key('setup-submit')));
    await tester.pumpAndSettle();
  ...
  testWidgets('supports switching language dynamically between English and Chinese', (
    tester,
  ) async {
